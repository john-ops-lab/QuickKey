...
1. 在仓库右侧打开 **Releases**。
2. 下载最新的 `QuickKey-macOS.zip`。
3. 解压后，将 `QuickKey.app` 拖入“应用程序”文件夹。
...
cd QuickKey
chmod +x scripts/build-app.sh scripts/validate.sh
./scripts/validate.sh
./scripts/build-app.sh
open dist/QuickKey.app
```

构建脚本会自动获取锁定版本的 `KeyboardShortcuts` 依赖，并在 `dist/` 中生成已临时签名的应用包。
